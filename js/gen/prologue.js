...
SC["start"]={img:"title",t:()=>'<h1>THE GOLDEN THREAD</h1><h2>a transmigration wuxia</h2>'+
 ...
 '<p class="small"><b>The complete edition</b> — Acts I–V. Five acts, nine endings, one golden thread.</p>',
...
SC["pro2"]={t:()=>P(`The wire snaps. White light eats the world.`)+DIV+P(`There is no pain. There is a place that is not a place: a darkness strung with threads beyond counting, each one a life, a river, a storm, a word. They hum. You drift among them, a loose stitch, and you understand with the calm of the dead that you have come off the Loom of one world and not yet been worked into another.`)+P(`One thread hangs nearest. It is gold. It is the exact color of your hair, and it is humming your name.`)+P(`First — what is your name?`)+`<p><input id="nm" maxlength="14" placeholder="Finn" value="${G.name||"Finn"}"></p>`,
 c:()=>[{id:"c0",l:`Reach out and GRASP the golden thread.`,do:()=>{G.name=($("nm").value.trim()||"Finn");G.f.grasped=true;addIns(1);},go:"pro3"},
  {id:"c1",l:`Go still, and let the thread come to you.`,do:()=>{G.name=($("nm").value.trim()||"Finn");addStat("heart",1);},go:"pro3"}]};
